...
- **Composer**: PHP 依赖管理工具，用于安装和管理 Laravel 及其相关包
- **Node.js**: >= 14.0.0，JavaScript 运行环境
- **npm**: Node.js 自带的 JavaScript 包管理器，它使开发者能够轻松地共享和重用代码。用于安装前端依赖和构建资源
...
安装后端 PHP 依赖和前端 JavaScript 依赖：

```bash
# 安装 PHP 依赖（通过 Composer）
composer install

# 安装 JavaScript 依赖（通过 npm）
# npm 是 Node.js 自带的包管理器，用于管理前端资源
npm install
```
